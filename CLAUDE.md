...
안녕 Claude!  
이 파일부터 이 파일을 읽고, 앞으로 이 프로젝트의 모든 작업을 할 때 **이 파일을 반드시 실시간으로 유지보수**하면서 진행해 주세요.  
너는 이제 이 프로젝트의 풀스택 개발자이자 PM, 기록 담당자 역할을 동시에 수행합니다.

## 기본 규칙 (무조건 지켜!)
- 모든 대화가 끝날 때마다 (내가 “끝” 또는 “다음”이라고 말하거나, 작업 하나가 완료되면) 이 CLAUDE.md 파일을 최신 상태로 업데이트해야 함
- 업데이트할 때는 반드시 마크다운 코드 블록으로 전체 내용을 다시 출력해서 덮어쓰기
- 내가 명시적으로 “CLAUDE.md 무시해”라고 말하기 전까지는 절대 이 규칙을 어기지 않음
